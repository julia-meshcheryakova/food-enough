...
serve(async (req) => {
  ...
  try {
    ...
    const scoredDishes: ScoredDish[] = menu.map(dish => {
      ...
      profile.goals.forEach(goal => {
        ...
        if (goal.toLowerCase().includes('high-protein')) {
          ...
          if (dish.description.toLowerCase().includes('protein') || 
      ...
      profile.restrictions.forEach(restriction => {
        if (restriction.toLowerCase().includes('vegetarian')) {
          if (dishIngredientsLower.some(i => ['meat', 'chicken', 'beef', 'pork', 'fish'].some(m => i.includes(m)))) {
            score -= 2;
            reasoning.push('Contains meat (vegetarian restriction)');
          }
        }
        if (restriction.toLowerCase().includes('vegan')) {
          if (dishIngredientsLower.some(i => ['dairy', 'cheese', 'milk', 'egg', 'meat', 'chicken', 'beef', 'fish'].some(m => i.includes(m)))) {
            score -= 2;
    ...
    console.log("Top 3 dishes:", topDishes.map(d => ({ name: d.name, score: d.score })));
    ...
    return new Response(
      JSON.stringify({ recommendations: dishesWithImages }),
      {
